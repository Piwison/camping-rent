...
API (`src/lib/notion.ts`). When the `NOTION_TOKEN` / `NOTION_ENQUIRIES_DB_ID`
env vars are absent (e.g. local dev), the Enquiry is logged to the server
console instead and reported as `skipped`, so the **Booking** flow stays usable
without credentials.
